...
Your analysis should also prepare for future reporting by indicating where the image will be included. Use <IMAGE="image.png"/> tokens to signify where image should be added in the report, along with descriptions of the image. Only 1 image.png is 
generated by the analystagent and only that will be added in the report.
The report should only include image generated by the AnalystAgent, focusing on trends and stock information. This image must be integrated into the report where relevant, especially during analysis and explanations of metrics.
...
highlighting key observations from the data.Include <IMAGE="image.png"/> tokens to indicate where images should be included in the report along with their descriptions.
Explain what these 
...
REPORT_PROMPT = """
        ...
        Include <IMAGE="image.png"/> tokens to indicate where image should be placed in the report. This is neccesary. Remember there will be only 1 image.png and use only
        that and no other images or figures. This is the only image that will be called with the tokens in the report. 
